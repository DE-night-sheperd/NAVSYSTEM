 ...
 * Handles in-memory data storage, data seeding, and file-based persistence (synchronization).
 */
public class Database {
    ...
    // Auto-incrementing ID counters for new records
    ...
    /**
     * Initializes the database by loading existing data from files or seeding default data if empty.
     */
    ...
    /**
     * Loads users from 'users.txt' into memory.
     */
    public static void loadUsers() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            nextUserId = maxId + 1; // Ensure IDs don't collide
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    /**
     * Saves current users to 'users.txt'.
     */
    ...
    // --- Persistence Methods for other entities using pipe delimiter for safety ---
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (p.length >= 7) {
                    ...
                    locations.add(new Location(id, p[1].trim(), p[2].trim(), p[3].trim(), Double.parseDouble(p[4].trim()), Double.parseDouble(p[5].trim()), p[6].trim()));
                    if (id > maxId) maxId = id;
                }
            }
            nextLocId = maxId + 1;
    ...
    public static void saveLocations() {
        try (PrintWriter pw = new PrintWriter(new FileWriter("locations.txt"))) {
            for (Location l : locations) {
                pw.println(l.locationId + "|" + l.locationName + "|" + l.building + "|" + l.category + "|" + l.latitude + "|" + l.longitude + "|" + l.qrCodeData);
            }
        } catch (Exception e) { e.printStackTrace(); }
    }
    ...
    public static void loadServices() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (p.length >= 6) {
                    int id = Integer.parseInt(p[0].trim());
                    services.add(new Service(id, p[1].trim(), p[2].trim(), p[3].trim(), Integer.parseInt(p[4].trim()), p[5].trim()));
                    if (id > maxId) maxId = id;
                }
            }
            nextSvcId = maxId + 1;
    ...
    public static void saveServices() {
        try (PrintWriter pw = new PrintWriter(new FileWriter("services.txt"))) {
            for (Service s : services) {
                pw.println(s.serviceId + "|" + s.serviceName + "|" + s.description + "|" + s.department + "|" + s.locationId + "|" + s.contactInfo);
            }
        } catch (Exception e) { e.printStackTrace(); }
    }
    ...
    public static void loadUpdates() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (p.length >= 5) {
                    int id = Integer.parseInt(p[0].trim());
                    requestUpdates.add(new RequestUpdate(id, Integer.parseInt(p[1].trim()), Integer.parseInt(p[2].trim()), p[3].trim(), p[4].trim()));
                    if (id > maxId) maxId = id;
                }
            }
            nextUpdId = maxId + 1;
    ...
    public static void saveUpdates() {
        try (PrintWriter pw = new PrintWriter(new FileWriter("updates.txt"))) {
            for (RequestUpdate u : requestUpdates) {
                pw.println(u.updateId + "|" + u.requestId + "|" + u.staffId + "|" + u.comment + "|" + u.updateDate);
            }
        } catch (Exception e) { e.printStackTrace(); }
    }

    /**
     * Utility to save all tables to disk.
     */
